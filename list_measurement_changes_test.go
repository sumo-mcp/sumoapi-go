...
func TestClient_ListMeasurementChanges(t *testing.T) {
	t.Run("list by rikishi ID", func(t *testing.T) {
		...
		transport := &mockTransport{
			validateRequest: func(req *http.Request) error {
				...
				g.Expect(req.URL.Path).To(Equal("/api/measurements"))
		...
		g.Expect(resp[0].Weight).To(Equal(158.0))
	})
	...
	t.Run("list by basho ID", func(t *testing.T) {
		...
		g.Expect(resp[0].Height).To(Equal(185.0))
		g.Expect(resp[1].RikishiID).To(Equal(789))
		g.Expect(resp[1].Weight).To(Equal(140.0))
	})
	...
	t.Run("context is propagated", func(t *testing.T) {
		...
		client := sumoapi.New(sumoapi.WithHTTPClient(&http.Client{Transport: transport}))
		_, err := client.ListMeasurementChanges(ctx, sumoapi.ListRikishiChangesRequest{})
